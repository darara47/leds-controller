...
#         self.pixelsValue = array.array("B", [0, 0, 0] * num_leds)
        self.pixelsValue = [[0 for i in range(3)] for i in range(300)]
        self.mode = mode
    ...
    def set_all(self, rgb_w, how_bright=None):
        
        with_W = len(rgb_w) == 4 and self.W_in_mode
        
        for index in range(self.num_leds):
            if with_W:
                self.set_pixel(index, rgb_w, how_bright)
            else:
                self.set_pixel(index, rgb_w, how_bright)
    
    def set_array(self, array_rgbbright):
        array_rgbbright = list(array_rgbbright)
        for index in range(len(array_rgbbright)):
            array_rgbbright[index] = list(array_rgbbright[index])
            if len(array_rgbbright[index]) == 3:
                array_rgbbright[index].insert(3, None)
            self.set_pixel(index, array_rgbbright[index][0:3], array_rgbbright[index][3])
    ...
    def set_pixel(self, pixel_num, rgb_w, how_bright=None):
        ...
        # colors in range 0-100
        bratio_100 = how_bright / 255.0
        red_100 = round(rgb_w[0] * bratio_100)
        green_100 = round(rgb_w[1] * bratio_100)
        blue_100 = round(rgb_w[2] * bratio_100)
        white_100 = 0
        # if it's (r, g, b, w)
        if len(rgb_w) == 4 and self.W_in_mode:
            white_100 = round(rgb_w[3] * bratio_100)

        # colors in range 0-255
        bratio_255 = how_bright / 100.0
        red_255 = round(rgb_w[0] * bratio_255)
        green_255 = round(rgb_w[1] * bratio_255)
        blue_255 = round(rgb_w[2] * bratio_255)
        white_255 = 0
        # if it's (r, g, b, w)
        if len(rgb_w) == 4 and self.W_in_mode:
            white_255 = round(rgb_w[3] * bratio_255)

        pix_value = white_100 << sh_W | blue_100 << sh_B | red_100 << sh_R | green_100 << sh_G
        ...
        if type(pixel_num) is slice:
            for i in range(*pixel_num.indices(self.num_leds)):
                ...
                self.pixelsValue[i] = (red_255, green_255, blue_255)
        else:
            self.pixels[pixel_num] = pix_value
            self.pixelsValue[pixel_num] = (red_255, green_255, blue_255)
